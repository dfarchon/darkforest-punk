...
// Sun rays were removed from SunRenderer (see comment in SunRenderer.ts line 25)
// This renderer is instantiated in Renderer.ts but never called
  ...
  implements SunRayRendererType
{
  ...
  public queueSunRayScreen(
    ...
    const facTop = 0.95 + Math.cos(angle + Math.PI) * 0.05;
    const facBot = 0.95 + Math.cos(angle) * 0.05;
    ...
    const effAngle = Math.sin(angle) * (Math.PI / 5);
  ...
  public queueSunRay(
    ...
    this.queueSunRayScreen(top, planet, center, radius, z, angle);
  }
  ...
  public setUniforms() {
    ...
    this.uniformSetters.time(time / 5);
  }
}

import type { Planet, Upgrade, UpgradeBranchName } from "@df/types";
...
const StatRowFilled = ({
  ...
  const getStat = (stat: string): number => {
    ...
    if (stat === "silverGrowth") {
      return mySelected[stat] * 60;
    } else {
  ...
  const statDiff = (stat: string): React.ReactNode => {
    ...
    if (diff < 0) {
      return <Red>{diff.toFixed(2)}</Red>;
    } else if (diff > 0) {
      return <Green>+{diff.toFixed(2)}</Green>;
    } else {
      return <Sub>0</Sub>;
    }
  };
  ...
  const updateClass = (stat: string): string => {
    if (getStat(stat) !== getStatFuture(stat)) {
      return "upgrade-willupdate";
    }
    return "";
  };
